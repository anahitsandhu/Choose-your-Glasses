...
def overlay_image_alpha(img, img_overlay, pos):
    x, y = pos #position of the glasses
    ...
    if overlay_image.shape[2] < 4:  
        overlay_image = np.concatenate([overlay_image, np.ones((*overlay_image.shape[:2], 1), dtype=np.uint8) * 255], axis=2)
...
def rank_glasses_for_oval_face(glasses_width, face_width, face_height, glasses_index):
    ...
    unique_factor = [0.8, 1.0, 1.2, 0.9,1.1,0.95][glasses_index]
...
def main():
    ...
    glasses_images = [
        ...
        load_glasses_image("glasses5.png"),
        load_glasses_image("glasses6.png")
    ]
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            glasses = cv2.resize(glasses_images[current_glasses], (glasses_width, glasses_height))
            
            glasses_pos_x = x + (w - glasses_width) // 2  # Centering the glasses
            glasses_pos_y = y + int(h / 6)  # Adjust height as needed to cover eyes better
            glasses_pos = (glasses_pos_x, glasses_pos_y)
